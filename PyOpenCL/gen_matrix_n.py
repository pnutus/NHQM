...
class GenMatrix:
    ...
    def allocate_space(self,type,contour):
        ...
        (k,w)=(contour)
        host_matrix=(numpy.array( \
            [i for i in range(self.size**2)])).astype(numpy.int32)
        host_k=(numpy.array( \
            [k[i//self.size] for i in range(self.size**2)])).astype(type)
        host_k_prim=( \
            numpy.array([k[i%self.size] for i in range(self.size**2)])).astype(type)
    ...
    def combine_kernel(self,arg=""):
        ...
        program_string=\
            ...
            self.potential+"\n"+\
            self.method
        self.kernel=ElementwiseKernel(self.ctx, \

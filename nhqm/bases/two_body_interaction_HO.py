...
def gen_matrix(eigvecs, Q):
    order = len(eigvecs)
    
    def V_func(i, j):
        return V_sep(i, j, Q)
    V_matrix = matrix_from_function(V_func, order, hermitian = True)
    
    def sep_M_func(i, j):
        return eigvecs[:,i].dot(V_matrix).dot(eigvecs[:,j])
    global sep_M
    sep_M = matrix_from_function(sep_M_func, order, hermitian = True)
    return sep_M

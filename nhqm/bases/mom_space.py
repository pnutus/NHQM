...
@sp.vectorize
def real_integrand(r, k, k_prim, V, l, j): #takes complex p, p_prim and returns real Re of integrand
...
def H_element(n, n_prim, problem, step_size, l = 0, j = .5):
    k, k_prim = [(x + 1)*step_size for x in (n, n_prim)] #diszretized momentum space basis
    diagonal = k**2 / (2 * problem.mass) * (k == k_prim)
    V = problem.potential
    integral, _ = fixed_quad(real_integrand, 0, 10, \

...
def find_existing_matrix( matrix_file_name_basis, matrix_file_path, matrix_file_format,  n = -1 , verbose = False ):
    ...
    for files in os.listdir( "." ):
        if files.rfind( matrix_file_name_basis ) == 0: #checks whether the filename starts with desired substring
            ...
            if temporary_dimension > existing_matrix_dimension:
                ...
                if existing_matrix_dimension >= n and n > 0:
                    break
...
def generate_succ_matrix_from_matrix( n, element_generating_function, existing_matrix, matrix_file_name_basis, matrix_file_path, matrix_file_format, verbose = False):
